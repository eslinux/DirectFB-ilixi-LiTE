...
#endif
{
    ...
    std::string name_ui("ui");
    Size s = PlatformManager::instance().getLayerSize(name_ui);
    ...
    printf("Application::Application FINISH \n");
}
...
void Application::setStylist(StylistBase* stylist)
{
...
//    printf("%s::%s<%d> \n", __FILE__, __FUNCTION__, __LINE__);
//    printf("getFontPack: %s \n", PlatformManager::instance().getFontPack().c_str());
...
#else
    // FIX stays in handleEvents until there are no more update events?
